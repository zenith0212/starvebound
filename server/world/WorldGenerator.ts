...
import { Biome } from './WorldBiomeResolver.js'
...
export class WorldGenerator {
  ...
  public generateWorld(config: any) {
    ...
    for (const map_obj_data of this.decodedMap) {
      switch (map_obj_data.type) {
        ...
        case 'gold': {
          ...
          if (obj.type == ObjectType.RIVER || obj.type == ObjectType.LAKE || obj.type == ObjectType.ISLAND)
            obj.isSolid = false
    ...
    for (const _Map of config) {
      switch (_Map[1]) {
        ...
        case 'FOREST':
          ...
          this.biomes.push(new Biome(__id, _Map[2], _Map[3], _Map[4], _Map[5]))
          break
        case 'isl': {
          const _i = _Map[3]
          const _j = _Map[4]
